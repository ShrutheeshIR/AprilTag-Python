...
static PyObject* apriltag_detect_and_getpose(apriltag_py_t* self,
                                 PyObject* args)
{
    ...
    PyArrayObject* rot = NULL;
    PyArrayObject* trans = NULL;
    ...
    if(!PyArg_ParseTuple( args, "O&ddddd",
                          PyArray_Converter, &image, &tagsize, &fx, &fy, &cx, &cy))
    {
        goto done;
    }
    ...
    for (int i=0; i < N; i++)
    {
        ...
        H = (PyArrayObject*)PyArray_SimpleNew(2, ((npy_intp[]){3,3}), NPY_FLOAT64);
        rot = (PyArrayObject*)PyArray_SimpleNew(2, ((npy_intp[]){3,3}), NPY_FLOAT64);
        trans = (PyArrayObject*)PyArray_SimpleNew(2, ((npy_intp[]){3,1}), NPY_FLOAT64);
        ...
        *(double*)PyArray_GETPTR1(xy_c, 1) = det->c[1];
        ...
        {
            for(int i=0; i<3; i++){
                // printf("%f", MATD_EL(det->H, j, i));
                *(double*)PyArray_GETPTR2(H,j,i) = MATD_EL(det->H, j, i);
            }
        }

        for(int j=0;j<3;j++){
            for(int i=0;i<3;i++){
                // printf("(%d,%d, %f)",j,i, MATD_EL(pose.R, j, i));
                *(double*)PyArray_GETPTR2(rot,j,i) = MATD_EL(pose.R, j, i);
            }
        }
        for(int i=0;i<3;i++)
            *(double*)PyArray_GETPTR2(trans,i,0) = MATD_EL(pose.t, i,0);
        ...
        PyTuple_SET_ITEM(detections_tuple, i,
                         Py_BuildValue("{s:i,s:f,s:i,s:f,s:O,s:O,s:O,s:O,s:O}",
                                       "hamming", det->hamming,
                                 ...
                                 PyObject* args)
{
    ...
    for (int i=0; i < N; i++)
    {
        ...
        *(double*)PyArray_GETPTR1(xy_c, 1) = det->c[1];
        ...
        PyTuple_SET_ITEM(detections_tuple, i,
                         Py_BuildValue("{s:i,s:f,s:i,s:O,s:O,s:O}",
                                       "hamming", det->hamming,
                                       "margin",  det->decision_margin,
                                       "id",      det->id,
                                       "center",  xy_c,
...
#include "apriltag_detect.docstring.h"
    ;
static const char apriltag_detect_and_getpose_docstring[] =
#include "apriltag_detect_and_getpose.docstring.h"
    ;
...
static PyMethodDef apriltag_methods[] =
    { PYMETHODDEF_ENTRY(apriltag_, detect, METH_VARARGS),
      PYMETHODDEF_ENTRY(apriltag_, detect_and_getpose, METH_VARARGS),
      {}
    //   PYMETHODDEF_ENTRY(apriltag_, detect_and_getpose, METH_VARARGS),
    //   {}
    };
